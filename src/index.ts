...
export function renderConformanceMatrix(report: ConformanceReport): string {
  ...
  const rows = report.adapters.map(({ adapterId, adapterVersion, hostVersion, capabilities, limitations, results }) => {
    const status = results.every(result => result.status === "pass") ? "pass" : "fail";
    const cells = phases.map(phase => capabilities.lifecycle[phase]);
    return `| ${adapterId} | ${adapterVersion} | ${hostVersion} | ${cells.join(" | ")} | ${capabilities.contextInjection} | ${capabilities.blocking} | ${status} | ${limitations.join("; ") || "none"} |`;
  });
  return [
